...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex justify-between items-center">
        <h2 class="text-3xl font-black text-white uppercase tracking-wider">Add Episode</h2>
        <a href="{{ route('admin.comics.episodes.index', $comic) }}" class="text-gray-400 hover:text-white transition-colors">&larr; Back to Episodes</a>
    </div>
    ...
    <div class="bg-neon-card border border-neon-border rounded-xl shadow-lg p-6">
        ...
        <form action="{{ route('admin.comics.episodes.store', $comic) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            
            <div>
                <label for="title" class="block text-sm font-bold text-gray-300 uppercase tracking-wide mb-2">Episode Title</label>
                <input type="text" name="title" id="title" value="{{ old('title') }}" required class="w-full bg-neon-dark border border-neon-border text-white rounded-lg focus:ring-neon-blue focus:border-neon-blue shadow-inner p-3" placeholder="e.g. Chapter 1: The Beginning">
            </div>

            <div>
                <label for="pdf_file" class="block text-sm font-bold text-gray-300 uppercase tracking-wide mb-2">PDF File</label>
